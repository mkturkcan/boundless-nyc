...
The photoreal pedestrians contain components created with Epic Games' MetaHuman. Epic's terms do not allow using
MetaHumans to train or enhance AI models. For frames meant as training data, start the server with
`--pedestrians procedural`, which uses the built-in procedural pedestrians instead.
